...
def run_camera_loop(video_path, cfg, FRAME_WINDOW, LOG_WINDOW, update_metrics_ui):
    ...
    while cap.isOpened():
        ...
        # PENYESUAIAN FONT ZONA (Scale 0.4, Thickness 1)
        cv2.polylines(frame, [STAFF_ZONE], True, (200, 100, 50), 2)
        cv2.putText(frame, "Zona Staf", (cfg["stf_x"], cfg["stf_y"] - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200, 100, 50), 1)

        cv2.polylines(frame, [CASHIER_ZONE], True, (53, 107, 255), 2)
        cv2.putText(frame, "Zona Kasir", (cfg["ksr_x"], cfg["ksr_y"] - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (53, 107, 255), 1)

        # --- Pelacakan BotSORT ---
        results = model.track(frame, persist=True, tracker="botsort.yaml", device='cpu', imgsz=640, conf=0.35, classes=[0], iou=0.4, verbose=False)
        ...
        if results[0].boxes is not None and results[0].boxes.id is not None:
            ...
            for box, track_id in zip(boxes, ids):
                ...
                # --- INISIALISASI MEMORI DINAMIS (Anti-Bocor) ---
